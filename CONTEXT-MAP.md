...
| CLI Protocol and Shared Integration Runtime | `packages/cli-protocol/**` | `packages/cli-protocol/AGENTS.md` | `packages/cli-protocol/CONTEXT.md` | None currently; system-wide `docs/adr/` | `docs/reference/conventions.md`, `docs/guardrails/effect-services-and-layers.md` | Provide retained HTTP, configuration, schema, error, and testing infrastructure while legacy CLI-only code awaits separate cleanup. |
...
- Changes to `packages/cli-protocol` affect both retained integration packages and Garage MCP transitively.
