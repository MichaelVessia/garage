...
const makeFake = Effect.gen(function* () {
  ...
  return { layer: Layer.mergeAll(Layer.succeed(CaddyApi, api), Layer.succeed(CaddyConfigFile, files)), reloads, reads }
})

it.effect('root command returns command tree and missing env remains recoverable', () =>
  Effect.gen(function* () {
    const fake = yield* makeFake
    const ok = yield* executeCaddy([]).pipe(Effect.provide(Layer.mergeAll(ConfigLayer, fake.layer)))
    const missing = yield* executeCaddy([]).pipe(Effect.provide(Layer.mergeAll(MissingConfigLayer, fake.layer)))
