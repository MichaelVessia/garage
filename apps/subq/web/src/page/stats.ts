...
export const CompletedOpenGoalForm = m('CompletedOpenGoalForm', { todayLocal: CalendarDate })
...
export const FetchStats = Command.define('FetchStats', {
  ...
  messages: [FetchStatsResult, FailedFetchStats],
  execute: ({ end, requestGeneration, start, timezone }) =>
    Effect.gen(function* () {
      const api = yield* Api
...
const ApplyPreset = Command.define('ApplyPreset', {
  args: { preset: Schema.Literals(['1m', '3m', '6m', '1y', 'all']), timezone: IanaTimezone },
  messages: [NavigatedStats],
  execute: ({ preset, timezone }) =>
    Effect.gen(function* () {
...
const NavigateStatsRange = Command.define('NavigateStatsRange', {
  args: { start: CalendarDate, end: CalendarDate },
  messages: [NavigatedStats],
  execute: ({ end, start }) =>
    pushUrl(statsRouter({ end: Option.some(end), start: Option.some(start) })).pipe(Effect.as(NavigatedStats())),
})

const OpenGoalForm = Command.define('OpenGoalForm', {
  args: { timezone: IanaTimezone },
  messages: [CompletedOpenGoalForm],
  execute: ({ timezone }) =>
    DateTime.now.pipe(Effect.map((now) => CompletedOpenGoalForm({ todayLocal: utcToLocalDateString(now, timezone) }))),
})

const SaveGoal = Command.define('SaveGoal', {
  args: {
    editingId: Schema.NullOr(GoalId),
  ...
  messages: [SucceededSaveGoal, FailedSaveGoal],
  execute: ({ editingId, goalWeightLbs, notes, startDate, targetDate }) =>
    Effect.gen(function* () {
      const api = yield* Api
...
const DeleteGoal = Command.define('DeleteGoal', {
  args: { goalId: GoalId },
  messages: [SucceededDeleteGoal, FailedSaveGoal],
  execute: ({ goalId }) =>
    Effect.gen(function* () {
      const api = yield* Api
...
export const updateStats = (
  ...
  return Match.value(message).pipe(
    ...
    Match.tagsExhaustive({
      ...
      CompletedOpenGoalForm: ({ todayLocal }) => [
        evo(model, {
          goalForm: () => ({
            editingId: null,
            error: null,
...
const makeViewStats = <ParentMessage>(h: HtmlBuilder<ParentMessage | StatsMessage>) => {
              ...
              [
                ...
                h.button(
                  [
                    ...
                    h.OnClick(ClickedStatsPreset({ preset: 'all' })),
                  ],
                  ['Reset']
                ),
              ]
              ...
              [
                ...
                h.div(
                  ...
                  [
                    ...
                    h.button(
                      [h.Class(button({ size: 'sm', variant: 'ghost' })), h.OnClick(ClickedEditGoal())],
                      ['Edit']
                    ),
                    h.button(
      ...
      [
        ...
        h.div(
          ...
          [
            ...
            viewCard(
              'Medications Used',
              viewBarChart(bundle.drugBreakdown.drugs.map((drug) => [drug.drug, drug.count]))
            ),
          ]
        ),
  ...
  return (model: StatsModel, unit: WeightUnit, range: StatsRange, timezone: IanaTimezone) =>
    h.div(
      [],
      [
...
export const viewStats = <ParentMessage>(
  model: StatsModel,
  unit: WeightUnit,
  range: StatsRange,
  timezone: IanaTimezone,
  h: HtmlBuilder<ParentMessage | StatsMessage>
) => makeViewStats(h)(model, unit, range, timezone)
