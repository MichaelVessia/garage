              ...
              Effect.map(
                (r) =>
                  new WeightLog({
                    ...
                    notes: r.notes !== null && r.notes.length > 0 ? Notes.make(r.notes) : null,
              ...
              Effect.map(
                (r) =>
                  new InjectionLog({
                    ...
                    source: r.source !== null && r.source.length > 0 ? DrugSource.make(r.source) : null,
                    dosage: Dosage.make(r.dosage),
                    injectionSite:
                      r.injection_site !== null && r.injection_site.length > 0
                        ? InjectionSite.make(r.injection_site)
                        : null,
                    notes: r.notes !== null && r.notes.length > 0 ? Notes.make(r.notes) : null,
                    scheduleId:
                      r.schedule_id !== null && r.schedule_id.length > 0
              ...
              Effect.map(
                (r) =>
                  new UserGoal({
                    ...
                    targetDate:
                      r.target_date !== null && r.target_date.length > 0 ? DateTime.makeUnsafe(r.target_date) : null,
                    notes: r.notes !== null && r.notes.length > 0 ? Notes.make(r.notes) : null,
                    isActive: r.is_active === 1,
                    completedAt:
                      r.completed_at !== null && r.completed_at.length > 0 ? DateTime.makeUnsafe(r.completed_at) : null,
                    createdAt: DateTime.makeUnsafe(r.created_at),
                    updatedAt: DateTime.makeUnsafe(r.updated_at),
                  })
              )
    ...
    const applyDataImport = (userId: string, plan: DataImportPlan) =>
      Effect.gen(function* () {
        ...
        for (const log of snapshot.data.injectionLogs) {
          const { source } = log
          const { injectionSite } = log
          const { notes } = log
          const { scheduleId } = log
          yield* sql`
