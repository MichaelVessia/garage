...
export const config: Effect.Effect<JsonObject, CaddyError, CaddyApi> = Effect.gen(function* () {
...
export const routes: Effect.Effect<ListResult<RouteSummary>, CaddyError, CaddyApi> = Effect.gen(function* () {
  const api = yield* CaddyApi
...
export const upstreams: Effect.Effect<ListResult<UpstreamRecord>, CaddyError, CaddyApi> = Effect.gen(function* () {
  const api = yield* CaddyApi
...
export const pkiCa: Effect.Effect<PkiCa, CaddyError, CaddyApi> = Effect.gen(function* () {
  const api = yield* CaddyApi
...
export const reload = Effect.fn('caddy.reload')(
  function* (nextConfig: JsonObject): Effect.fn.Return<ReloadResult, CaddyError, CaddyApi> {
    const api = yield* CaddyApi
