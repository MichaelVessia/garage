...
const makeApiLayer = Effect.gen(function* () {
  ...
  const api = AutocaliwebApi.of({
    ...
    catalog: () =>
      Effect.succeed({
        count: 1,
        records: [{ title: 'Alphabetical Books', href: '/opds/books' }],
      }),
    books: (options) =>
    ...
    shelves: () =>
      Effect.succeed({
        count: 1,
        records: [{ title: 'Favorites', href: '/opds/shelf/1' }],
      }),
  })
  return {
    layer: Layer.succeed(AutocaliwebApi, api),
    bookOptions,
    recentOptions,
    searchOptions,
  }
...
it.effect('operations do not preflight config and forward options to AutocaliwebApi', () =>
  Effect.gen(function* () {
    ...
    assert.strictEqual((yield* status.pipe(Effect.provide(fake.layer))).title, 'Fixture Catalog')
    assert.strictEqual((yield* version.pipe(Effect.provide(fake.layer))).title, 'Fixture Catalog')
    assert.strictEqual((yield* stats.pipe(Effect.provide(fake.layer))).books, 1)
    assert.strictEqual((yield* catalog.pipe(Effect.provide(fake.layer))).records[0]?.title, 'Alphabetical Books')
    assert.strictEqual(
      (yield* books({ limit: 3 }).pipe(Effect.provide(fake.layer))).records[0]?.title,
      'Fixture Novel One'
    )
    assert.strictEqual(
      (yield* recent({ limit: 2 }).pipe(Effect.provide(fake.layer))).records[0]?.title,
      'Fixture Novel Two'
    )
    assert.strictEqual(
      (yield* search({ query: 'fixture', limit: 4 }).pipe(Effect.provide(fake.layer))).records[0]?.title,
      'Fixture Novel One'
    )
    assert.strictEqual((yield* bookInfo({ uuid: 'book-uuid' }).pipe(Effect.provide(fake.layer))).uuid, 'book-uuid')
    assert.strictEqual((yield* shelves.pipe(Effect.provide(fake.layer))).records[0]?.title, 'Favorites')
